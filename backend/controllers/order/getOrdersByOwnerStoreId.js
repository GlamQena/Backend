  ...
  try {
    ...
    const storeOrders = orders.map(order => {
      ...
      return {
        ...
        store_payout: order.profit_breakdown?.stores_payout?.find(
    ...
    const summary = {
      ...
      total_revenue: storeOrders.reduce((sum, order) => sum + (order.store_subtotal || 0), 0),
      total_payout: storeOrders.reduce((sum, order) => sum + (order.store_payout || 0), 0),
