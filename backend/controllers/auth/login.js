...
const loginController = async (req, res) => {
  try {
    ...
    let { usernameOrEmail, password, activationCode, rememberMe, session_id } = req.body;
    ...
    if(token){
      ...
      if(user_id.toString() !== user._id.toString() || user.role !== role || (role !== "admin" && role !== "storeOwner")){
        return res.status(401).json({message: "Invalid token user"});
      }

      if(!activationCode){
        return res.status(400).json({message: "activation code is required for first login"});
      }

      activationCode= activationCode.trim();
      if(activationCode.length !== 6){
        return res.status(400).json({message: "activation code must be 6 digits length"});
      }

      //verify the account activation code
      const foundCode= await otpModel.findOne({userId: user_id, otpCode: activationCode, for: "activateAccount"});

      if(!foundCode || foundCode.otpExpiry < Date.now() || foundCode.isVerified || foundCode.isActive === false){
        return res.status(400).json({message: `Invalid or expired activation code ${foundCode ? `(attempts left: ${3 - (++foundCode.otpAttempts)})` : ""}`});
      }
      ...
      const foundResetPassCode= await otpModel.findOne({userId: user_id, for: "resetPassword"});
      ...
      sendEmail({to: user.email, subject: "Reset Password Code", text: `Your password reset code is: ${resetPassCode}`});
      //don't required to await for sendEmail although it's async for the response to not be late
    }
    ...
    res.status(200).json({
      message: cartMergeResult?.merged
        ...
        : resetPassCodeCreated? "Login successful. Please reset your password with the new code sent to your email."
        : "Successful login...",
      user: userData,
      accessToken,
      refreshToken,
